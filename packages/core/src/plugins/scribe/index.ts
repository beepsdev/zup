 ...
 * incident, the scribe writes a markdown playbook describing what happened,
 * what caused it, and what fixed it — into a directory of plain .md files
 * that humans can read, edit, delete, and commit to git.
 *
 ...
 * are injected into LLM context (e.g. by the investigation-orienter).
 *
 * This is the file-based alternative to the `historian` plugin: no SQLite,
 * no embeddings — matching relies on generous trigger keywords generated at
 * write time (by the LLM when available).
 */
...
import { parsePlaybook, loadPlaybooksFromDir } from '../../playbook';
import type { AgentContext, LoopResult } from '../../types/index';
import type { ScribeOptions, IncidentNarrative, IncidentRecord } from './types';
...
export const scribe = (options: ScribeOptions = {}) => {
  ...
  const plugin = definePlugin({
    ...
    init: async (ctx: AgentContext) => {
      ...
      if (loaded.length > kept.length) {
        ctx.logger.warn(
          `[scribe] ${loaded.length} incident files in ${dir}, loading newest ${kept.length} (maxIncidents)`
        );
      }
      ...
      ctx.logger.info(`[scribe] Loaded ${kept.length} recorded incident(s) from ${dir}`);
    },
    ...
    onLoopComplete: async (loopResult: LoopResult, ctx: AgentContext) => {
      ...
      if (!loopResult.actionResults.some(r => r.success)) {
        ctx.logger.debug('[scribe] Skipping record: no successful actions');
        return;
      }

      if (loopResult.decision.confidence < minConfidence) {
        ctx.logger.debug(
          `[scribe] Skipping record: confidence ${loopResult.decision.confidence} < ${minConfidence}`
        );
        return;
      }
      ...
      if (!includeHighRisk && (risk === 'high' || risk === 'critical')) {
        ctx.logger.debug(`[scribe] Skipping record: ${risk}-risk action`);
        return;
      }
      ...
      const playbook = parsePlaybook(markdown, { source: 'filesystem', sourcePath: filePath });
      ...
      ctx.logger.info(`[scribe] Recorded incident: ${record.narrative.description} -> ${filePath}`);
    },
  });
...
async function buildNarrative(
  ...
  if (useLLM && ctx.llm) {
    try {
      ...
      ctx.logger.warn('[scribe] LLM narrative generation failed, using fallback:', error);
    }
  }
...
function llmPrompt(loopResult: LoopResult): string {
  ...
  return [
    ...
    `Findings: ${situation?.assessments?.flatMap(a => a.findings).join('; ') || 'none'}`,
    `Contributing factor: ${situation?.assessments?.map(a => a.contributingFactor).filter(Boolean).join('; ') || 'unknown'}`,
...
function fallbackNarrative(loopResult: LoopResult): IncidentNarrative {
  ...
  // With no orienters the situation summary is a placeholder — the decision
  // rationale describes the incident better in that case.
  const hasAssessments = (situation?.assessments?.length ?? 0) > 0;
  const summary = hasAssessments && situation?.summary
    ? situation.summary
  ...
  const text = [
    summary,
    ...(situation?.assessments?.flatMap(a => a.findings) ?? []),
    ...(situation?.assessments?.map(a => a.contributingFactor).filter((f): f is string => !!f) ?? []),
...
export function renderIncidentMarkdown(record: IncidentRecord): string {
  ...
  const findings = situation?.assessments?.flatMap(a => a.findings) ?? [];
  const factors = situation?.assessments
  ...
  const lines = [
    ...
    '## What happened',
    '',
    situation?.summary ?? 'No situation summary recorded.',
